...
RegisterServerEvent('ms-lumberjack:server:workvan', function(workVanPrice)
    local source = source
...
RegisterServerEvent('ms-lumberjack:server:returnworkvan', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('ms-lumberjack:server:addLog', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)

    local logCount = Player.Functions.GetItemByName('ms_log')
    ...
    exports.ox_inventory:AddItem(source, 'ms_log', 1)
    notifyPlayer(source, Lang.addedLog, 'success')
end)

RegisterServerEvent('ms-lumberjack:server:removeLog', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)

    if Player then
        exports.ox_inventory:RemoveItem(source, 'ms_log', 1)
    end
end)

RegisterServerEvent('ms-lumberjack:server:deliverypaper', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if Player then
        ...
        if exports.ox_inventory:Search(source, 'count', 'ms_deliverypaper') > 0 then
            notifyPlayer(source, Lang.alreadyHaveDeliveryPaper, 'error')
            return
        end

        if exports.ox_inventory:AddItem(source, 'ms_deliverypaper', 1) then
...
RegisterServerEvent('ms-lumberjack:server:sellinglog', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if Player then
        ...
        if exports.ox_inventory:Search(source, 'count', 'ms_log') < 1 then
        ...
        if exports.ox_inventory:RemoveItem(source, 'ms_log', 1) then
            exports.ox_inventory:AddItem(source, 'cash', cashReward)

            if playerLogSales[source] >= Config.maxLogs then
                if exports.ox_inventory:RemoveItem(source, 'ms_deliverypaper', 1) then
                    TriggerClientEvent('ms-lumberjack:client:resetTimmyTask', source)
                end
                playerLogSales[source] = 0
            end
...
RegisterServerEvent('ms-lumberjack:server:choptree', function()
    local source = source
    -- Because you are going to be able to carry till your inventory is full (Don't ask why because Im stupid alright)
    if exports.ox_inventory:CanCarryItem(source, 'ms_choppedlog', 1) then
        if exports.ox_inventory:AddItem(source, 'ms_choppedlog', 1) then
...
RegisterServerEvent('ms-lumberjack:server:craftinginput', function(argsNumber, logAmount)
    local source = source
    ...
    if slot == 1 then
        itemToReceive = 'ms_woodplank'
        totalItems = itemCount * Config.receive.ms_woodplank
    elseif slot == 2 then
        itemToReceive = 'ms_woodhandles'
        totalItems = itemCount * Config.receive.ms_woodhandles
    elseif slot == 3 then
        itemToReceive = 'ms_firewood'
        totalItems = itemCount * Config.receive.ms_firewood
    elseif slot == 4 then
        itemToReceive = 'ms_toyset'
        totalItems = itemCount * Config.receive.ms_toyset
    else
        if Config.debug then
    ...
    if exports.ox_inventory:CanCarryItem(source, itemToReceive, totalItems) then
        if exports.ox_inventory:RemoveItem(source, 'ms_choppedlog', itemCount) then
            Wait(7)
...
RegisterServerEvent('ms-lumberjack:server:sellitem', function(args)
    local source = source
    ...
    if itemType == 'ms_firewood' or itemType == 'ms_woodplank' then
        sellingLocation = vector3(Config.seller1.x, Config.seller1.y, Config.seller1.z)
    elseif itemType == 'ms_toyset' or itemType == 'ms_woodhandles' then
